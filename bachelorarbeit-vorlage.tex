...
\end{Definition}
We always denote the elements of $\C\G$ by a linear combination $\sum_{i \in I} \tilde f_i \odot_{\C \G} \tilde \Phi_i $ where $I$ is a finite set, $\Phi_i$ is a measurable edge and $f_i \in L^\infty(\G_0)$. One can show that each element of $\C \G$ can be represented in such a way. Be aware that this representation is non-unique because there might be several pairs of operators such that their respective compositions are equal. In addition for $f \in L^\infty(\G)$ it holds that $\widetilde{\frac{1}{2}f} + \widetilde{\frac{1}{2}f} = \tilde{f}$. \todo{Evtl. könnte ich dazu noch ein bisschen was schreiben}
\begin{Example} \label{groupoid_ring_of_groups}
	...
	\begin{align*}
		...
		= &~ (\tilde{\Phi}_1 \circ \tilde{\Phi}_2)(F)(\gamma) \\
		= &~ \tilde \Phi_1 (\alpha \mapsto F(\alpha \odot_{\G}\Phi_2^{-1}(\bullet)))(\gamma) \\
...
\begin{Definition} \label{vNd_of_groupoids}
	...
	\end{align*}
\end{Definition}
This definition of a von Neumann dimension is very similar to the one on $\ell^2 G$ given in \ref{vNd} when you consider the fact stated in \ref{vNd_for_subsets}. Again this definition is independent of the chosen projection. \todo{unabhaengigkeit}
...
\begin{Lemma} \label{trgroupoid} \todo{anders herum}
...
\begin{proof}
	...
	\begin{align*}
		...
		=&~ \widetilde{P(\hat{\rho}(\gamma_1)(\hat{x_2}))}(~\widetilde{\bar{\gamma_1}}(F)~)~(\alpha) \\
		=&~ \widetilde{P(\hat{\rho}(\gamma_1)(\hat{x_2}))}(~\beta \mapsto F(\beta \odot_{\G (\rho)} \bar\gamma_1^{-1}(r(\beta)))~)~(\alpha) \\
		=&~ F(\alpha \odot_{\G (\rho)} \bar\gamma_1^{-1}(r(\alpha))) ~ \cdot ~ P(\hat{\rho}(\gamma_1)(\hat{x_2}))~(r(\alpha))\\
		=&~ F(\alpha \odot_{\G (\rho)} \bar\gamma_1^{-1}(r(\alpha))) ~ \cdot ~ P(\hat{x_2} \circ \rho(\gamma_1^{-1}))~(r(\alpha)) \\
		=&~ F(\alpha \odot_{\G (\rho)} \bar\gamma_1^{-1}(r(\alpha))) ~ \cdot ~ \hat{x_2} ~(~\rho(\gamma_1^{-1})(r(\alpha))~)	\\
		=&~ F(\alpha \odot_{\G (\rho)} \bar{\gamma_1}^{-1}(r(\alpha))) ~\cdot~ \hat{x_2}(r(\bar{\gamma_1}^{-1}(r(\alpha))))	
	\end{align*}
	and
	\begin{align*}
		 &~ (\widetilde{\bar{\gamma_1}} \odot_{\C\G(\rho)} \widetilde{P(\hat{x_2})}) ~(F) ~(\alpha) \\
		=&~ (\widetilde{\bar{\gamma_1}} \odot_{\C\G(\rho)} \tilde{\hat{x_2}})~(F)~(\alpha) \\
		=&~ \widetilde{\bar{\gamma_1}} (~ \tilde{\hat{x_2}}(F)~)~ (\alpha) \\
		=&~ \widetilde{\bar{\gamma_1}} (~ \beta \mapsto F(\beta) \cdot \hat{x_2}(r(\beta))~)~ (\alpha) \\
		=&~ F(\alpha \odot_{\G (\rho)} \bar{\gamma_1}^{-1}(r(\alpha))) ~\cdot~ \hat{x_2}(r(\alpha \odot_{\G (\rho)} \bar{\gamma_1}^{-1}(r(\alpha)))) \\
		=&~ F(\alpha \odot_{\G (\rho)} \bar{\gamma_1}^{-1}(r(\alpha))) ~\cdot~ \hat{x_2}(r(\bar{\gamma_1}^{-1}(r(\alpha))))
	\end{align*}
	...
	\begin{align*}
		...
		=&~ \langle ~(\sum_{(\hat{x}, \gamma) \in \hat{X} \rtimes_{\hat{\rho}} \Gamma} c_{(\hat{x}, \gamma)} \cdot (\hat{x_i}, \gamma_i))~(e_{\hat{X} \rtimes_{\hat{\rho}} \Gamma}),~ e_{\hat{X} \rtimes_{\hat{\rho}} \Gamma} \rangle \\
		=&~ c_{(e_{\hat{X}}, e_\Gamma)}.
	\end{align*}
	We can use \ref{trgroupoid} to get
	\begin{align*}
		 &~ \tr(~ P \otimes 1 (\sum_{(\hat{x}, \gamma) \in \hat{X} \rtimes_{\hat{\rho}} \Gamma} c_{(\hat{x}, \gamma)} \cdot (\hat{x_i}, \gamma_i))~) \\
	...
	\end{align*}
	Now let $\tilde{f}  \tilde{\bar{\gamma}} \in \C \G(\rho)$ be an element of the groupoid ring of an arbitrary action groupoid. Then for $\phi_1, \phi_2 \in L^2 \G(\rho)$ it holds that
	\begin{align*}
		 &~ \langle ~\tilde{f} \tilde{\bar{\gamma}}  (\phi_1),~ \phi_2 ~\rangle_{L^2 \G(\rho)} \\
		...
		=&~ \langle ~\phi_1,~ \tilde{f'} \widetilde{\overline{\gamma^{-1}}}  (\phi_2) ~\rangle_{L^2 \G(\rho)}
	\end{align*}
	with
	\begin{align*}
	...
	\begin{align*}
		...
		=&~ \sum_{(\hat{x}, \gamma) \in \hat{X} \rtimes_{\hat{\rho}} \Gamma} c_{(\hat{x}, \gamma)} \cdot \widetilde{P(\hat{\rho}(\gamma^{-1})(\hat{x}^{-1}))} \odot_{\C\G(\rho)} \widetilde{\overline{\gamma^{-1}}} \\
...
\main*
\begin{proof}\todo{selbstadjungiert}
	With \ref{pontr_vNd} it remains to show that the map $S$ from \ref{groserSatz} is contained in $P \otimes 1(\Q (\hat{X} \rtimes_{\hat{\rho}} \Gamma))$.\footnote{compare \ref{def_arising}}
	
	Because our Turing dynamical system is computable, $X$ is an infinite product of $\2$. The group $\widehat{\2}$ consists of only two elements. The neutral element, which maps all of $\2$ to $1$ and the element which maps $\0$ to $1$ and $\1$ to $-1$. Therefore $\widehat{\2}$ is isomorphic to $\2$. In addition one can see \todo{Quelle} that the Pontryagin dual of an infinite product is an infinite sum. Therefore 
	\begin{align*}
		 &~\hat{X} \\
	...
	\begin{align*}
		...
		\end{cases}.
	\end{align*} 
	and $e_{\hat{X}} \in \hat{X}$ be the neutral element, i.e. the map which send every element of $X$ to $1$. Let $e_{\Gamma}$ denote the neutral element of $\Gamma$. Then it holds that
	\begin{align*}
	...
	\begin{align*}
		P \otimes 1(\frac{1}{2} (e_{\hat{X}}, e_{\Gamma}) - \frac{1}{2}(g_i, e_{\Gamma})) = \tilde{\chi_{Z_i}}
	\end{align*}
	with
	\begin{align*}
	...
	\begin{align*}
		X_i = \{(x_j)_{j \in J} \in X \ | \ x_k = \0 \ \forall k \in I_0, x_h = \1 \ \forall h \in I_1\}.
	\end{align*}
	where $I_0, I_1 \subset J$ are finite subsets, be one of subsets defined by $(T_X)$. Then 
	\begin{align*}
		\widetilde{\chi_{X_i}} = \widetilde{\Pi_{k \in I_0} ~ \chi_{Y_k} \cdot \Pi_{h \in I_1} ~\chi_{Z_h}}
	\end{align*}
	lies in $P \otimes 1(\Q (\hat{X} \rtimes_{\hat{\rho}} \Gamma))$ because $P \otimes 1$ preserves the multiplication. Because of the definition of $P \otimes 1$ all $\tilde{\bar{\gamma}}$ from the definition of $S$ lie also in the above image.
\end{proof}
\todo{tilde, bar, dach}
...
\end{align*}
Because $\Z$ acts only on the first factor and $\Aut(\2^n)$ acts only on the second factor of $X$ and $\Z$ acts by shift we get \todo{Aut liegt nur in natuerliche Weise drinnen}
\begin{align*}
...
\begin{proof}
	...
	Let $0. a_1 a_2 \ldots a_k = \frac{1}{m}$ be the (unique) finite binary expansion of $\frac{1}{m}$. We construct a Turing machine $T$ with $k+3$ states. These states are
	\begin{itemize}
	...
	\begin{align*}
		...
		\delta(s_i, \1) =&~ \begin{cases}
			(s_A, \1, 0) &~ \text{if} \ a_i = 1 \\
			(s_R, \1, 0) &~ \text{if} \ a_i = 0
		\end{cases}
	\end{align*}
	...
	\begin{figure}[]\todo{akz rej Pfeile krumm}
		\centering
		\begin{tikzpicture}[shorten >=1pt,on grid,auto]
		\node[state] (0) {$s_I$};
...
\begin{Lemma} \label{frac}
	For every $m \in \N$ with $m \neq 1$ there is a binary expansion of $\frac{1}{m}$ of the form $0. a \overline{b}$ where $a$ is a nonrepeating part, $b$ is a repeating part and the lengths of $a$ and $b$ are both finite.
\end{Lemma}
...
\begin{proof}
	...
	\begin{align*}
		...
		\end{cases}
	\end{align*}
	which takes care of the repeating part of $\frac{1}{m}$. The rest of the transition function $\delta$ stays the same as in \ref{1TM}. Figure \ref{Graphik_zweite_TM} shows an example of this Turing machine for the number $\frac{1}{m} = 0.10 \overline{101}$.\todo{zahl, die es gibt}
	...
	We can see that this Turing machines does not stop for every input, because if we have only zeros on the right side of our starting point, we never arrive at $s_A$ or $s_R$. But because the set $[\underline{x} \ \0 \ \0 \ldots][\sigma]$ (with $x \in \2$ and $\sigma$ an arbitrary state) has measure zero the resulting Turing dynamical system still stops on any configuration. The rest follows exactly like in the proof of \ref{1TM} because the infinite sum of measures converges.
\end{proof}

We can now start to prove \ref{mainTh}. \todo{restate}

\begin{proof}
	Because of of the additivity of numbers arising from a group proven in \ref{add} it suffices to show that for every $m \in \N$, $\frac{1}{m}$ arises from $L$. Let $m \in \N$  and $(T_X)$ be the Turing dynamical system constructed in the proof of \ref{2TM}. First we see that
	\begin{align*}
		 &~ \mu (I) - \Omega_1(X) \\
	...
	\end{align*}
	is an $\ell^2$-Betti number of $L \times (\2^{k'} \rtimes_{\rho|_{\Rot}} \Rot)$. With the multiplicativity \todo{evtl nicht bewiesen}proven in \ref{mult} it follows that 
	\begin{align*}
...
\end{Notation}
\begin{Theorem} \todo{warum irrational}
	The number
	\begin{align*}
...
\begin{proof}
	...
	Therefore in order to accept we need to have $j \cdot (j + 1) = j^2 + j$ \todo{2j?} many $\0$ on the third tape. Therefore it holds that
	\begin{align*}
	...
	It remains to show that $(T_X)$ is computable. \todo{wiederholen, was man braucht} First let us look at the set 
	\begin{align*}
		\{x \in X ~|~ T_X^\infty(x) \notin A \cup R\}.
	\end{align*}
