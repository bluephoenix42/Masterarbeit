...
\begin{Definition}
	...
	\begin{align*}
		X_i = \{(x_j)_{j \in J} \in X \ | \ x_k = \0 \ \forall k \in I_1, x_h = \1 \ \forall h \in I_2\, \ I_1, I_2 \subset J \ \text{finite}\}.
	\end{align*}
...
\begin{Lemma}
	...
	\begin{align*}
		M = \{(x_j)_{j \in J} \in X \ | \ x_k = 0 \ \forall k \in I_1, x_h = 1 \ \forall h \in I_2\, \ I_1, I_2 \subset J \}
	\end{align*}
	it holds that
	\begin{align*}
	\mu (M) = \begin{cases}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[shorten >=1pt,on grid,auto]
		...
		;
	\end{tikzpicture}
	\caption{Procedure to prove \ref{mainTh}}
	\label{Masterplan}
\end{figure}
...
We now come to the definition of the groupoid ring. We want it to be a ring of operators of	$L^2(\G)$ \todo{Quelle fuer l2}, such that it is in a way generated by measurable edges. For a measurable edge $\Phi$ we define a operator $\tilde \Phi$ through
\begin{align*}
...
\begin{proof}
	...
	To prove the statement about the orbits we restrict the groupoid $\G (T_X)$ to a groupoid $\G (T_X)^\#$ in such a way that $T_X$ is injective and that for every point $x \in \G (T_X)^\#$, $G (T_X)^\# x$ contains a point in $A \cup R$. Because $(T_X)$ stops at any configuration and has disjoint accepting chains $\G (T_X)^\#$ still has measure $1$. We can see that the map $T_X$ applied to $\G (T_X)^\#$ is measure-preserving.\todo{will man das noch zeigen?}
