...
\begin{Definition}
	...
	\begin{align*}
		\tr_{\mathcal{N}(G)}\colon \mathcal{N}(G) \to \C, A  \mapsto \langle A(1 \cdot e_G), 1 \cdot e_G \rangle
	\end{align*}
...
\end{Definition}
\begin{Definition}
	Let $H$ be a Hilbert $\mathcal{N}(G)$-module with a corresponding embedding \\ $\iota\colon H \to \ell^2(G)^n$ and let $pr_{\iota(H)}\colon \ell^2(G)^n \to H$ be a corresponding $G$-equivariant projection. Let $\iota_i\colon \ell^2(G) \to \ell^2(G)^n, y \mapsto (x_1, \ldots, x_n)$ with $x_j = \delta_{i,j} \cdot y$ be the inclusion of the i-th $\ell^2(G)$ and $pr_i\colon \ell^2(G)^n \to \ell^2(G), (x_1, \ldots, x_n) \mapsto x_i$ the corresponding projection. Then 
	\begin{align*}
...
\begin{Remark}\label{vNd_for_subsets}
	...
	\dim_{\mathcal{N}(G)}(H) = \tr_{\mathcal{N}(G)}(pr_H) = \langle pr_H (1 \cdot e_G), 1 \cdot e_G \rangle.
	\end{align*}
\end{Remark}
...
We now come to the definition of the groupoid ring. We want it to be a ring of operators of	$L^2(\G)$, \todo{Quelle fuer l2} such that it is in a way generated by measurable edges. For a measurable edge $\Phi$ we define an operator $\tilde \Phi$ through
\begin{align*}
...
In addition for a map $f \in L^\infty (\G_0)$ we also define an operator $\tilde f$ on $L^2(\G)$ through
\begin{align*}
...
\begin{Definition} \label{vNd_of_groupoids}
	Let $\G$ be a groupoid, $X \subset L^2 \G$ and $pr_X\colon L^2 \G \to X$ be projection onto $X$. The \emph{von Neumann dimension} of $X$ is defined through
	\begin{align*}
...
\end{Definition}
Lets first look at the Turing dynamical system corresponding to a read-only Turing machine as constructed in \ref{roTMtoTDS}. If we can ensure that the resulting Turing dynamical system has disjoint accepting chains and stops at any configuration we can ensure that it is computing. By using \ref{HS} we see that $\mu (I) - \Omega_1(T_X)$ is an $\ell^2$-Betti number arising from $\hat{X} \rtimes_{\hat{\rho}} \Gamma$. We then get
\begin{align*}
...
\begin{Remark}
	...
	\begin{align*}
		M = \{((a_i)_{i \in \Z}, (b_i)_{i \in \Z}, (c_i)_{i \in \Z}) \in (\2^\Z)^3 | a_{-\alpha} = l_{-\alpha}, \cdots, a_\beta = l_\beta,\\
...
\begin{proof}
	...
	\begin{figure}[]
		...
		\node[state] (2) [right = 3 of 1] {$s_2$};
		\node[state] (3) [right = 3 of 2] {$s_3$};
		\node[state] (4) [above = 3 of 2] {$s_A$};
		\node[state] (5) [below = 3 of 3] {$s_R$};
