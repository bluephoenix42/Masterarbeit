...
\begin{Example}\label{TMtoTDS}
	Now let  $T=(S,\delta, \tilde{A}, \tilde{R}, \tilde{I})$ be a Turing machine. We will show how to emulate it with a Turing dynamical system. Lets first fix notation. For a subset $M$ of a set $N^\Z$ with $M = \{(n_i)_{i \in \Z} \in N^\Z | n_{-k} = m_{-k}, \cdots, n_l = m_l \}$ \todo{erklären, was es mit k und l auf sich hat}we simply write $[m_{-k} \cdots \underline{m_0} \cdots m_l]$ and $[m_{-k} \cdots \underline{m_0} \cdots m_l][\sigma]$ for $M \times \{\sigma\}$. We define $X = \2^\Z \times S$ and $\Gamma = Bij(S) \times \2 \times \Z$. \todo{in unserem fall ist sigma usw., S Menge, Bij Grupper erwähnen}
	\todo{Stimmt das?}
	...
	\begin{itemize}
		...
		\item The generator $1$ of the group $\Z$ acts also on the $\Z / 2\Z^\Z$ part by shifting every element 1 to the right, e.g. increasing the index of every element by 1. \todo{nochmal ausführlich}
	\end{itemize}
	...
	\end{align*}
	For $X_i = [\underline{x}][\sigma]$ we get $\delta(\sigma, x) = (\tilde{\sigma}, \alpha, \beta)$.\todo{sigma und x reihenfolge Satz neu} Let $\tau \in Bij(S)$ map $\sigma$ to $\tilde{\sigma}$ and $y \in \2$ be $y=x-\alpha$, i.e. an Automorphism \todo{Quatsch Operation} which maps $x$ to $\alpha$. Our element $\gamma_i$ corresponding to $X_i$ is given by 
	\begin{align*}
	\gamma_i = \begin{cases}
...
\end{Example} \todo{Abschlusssatz, reihenfolge vertauschen}
...
\end{Definition}
\todo{Will ich das wirklich so nennen? Ist X richtig definiert? Warum cont group autos? Indexmenge J beim Produkt, andere Xi defintion}
\todo{maß}
...
\end{Definition}
\todo{am ende muss noch invertierung ans phi}
