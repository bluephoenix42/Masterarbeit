...
\begin{Definition} \label{GR}
	...
	\begin{align*}
		\sum_{h \in H} \alpha_h h \cdot_{RG} \sum_{k \in K} \beta_k k = \sum_{h \in H} \sum_{k \in K} \alpha_h \beta_k h \cdot_G k
	\end{align*}
...
\end{Remark}\todo{G-equivariant erwaehnen}
\begin{Lemma}
...
\begin{proof}
	...
	\end{align*}
	which is a map from $C_n^{(2)}(X)$ to $C_n^{(2)}(X)$. The differential can be written as a matrix $A' \in \Z G$ and therefore $\Delta_n^{(2)}$ also lies in $\Z G$. It remains to show that $\ker(\Delta_n^{(2)}) = H_n^{(2)}$ holds. \todo{ZG^n times n}
...
\end{proof}
\todo{ker(delta) = homology zeigen? Graphik und etwas genauer? oben hinschreiben, dass man nur die erste zeile macht}
\begin{Remark}
...
\todo{Unterschied klar machen, lieber Liste, Ind klar machen, I klar machen?}
...
Let $\Ind\colon X \to \N$ be the map which assigns to each $x \in X$ the corresponding index of the $X_i$ it is contained in.
\begin{Definition}
	 The map 
	 \begin{align*}
...
\begin{Definition}
	...
	If in addition the action of $\Gamma$ is by continuous group automorphisms and $(T_X)$ stops on any configuration, has disjoint accepting chains and does not restart it is called a \emph{computing} Turing dynamical system.
\end{Definition}
\todo{auflistung}
\begin{Remark}
...
\end{Definition}
Of course the Turing dynamical system constructed in \ref{TMtoTDS} is not computing in most cases. But for many read-only Turing machines a computing Turing dynamical system can be constructed in a similar way. In fact we will only use read-only Turing machines. \todo{geht auch fuer nicht readonly, warum geht das nicht mit der obigen def}
\begin{Example} \label{roTMtoTDS}
	...
	We can then assume that $X = \2^\Z \times \2^n$. We regard every element of $\2^n$, which does not correspond to an actual state, as a ``dummy state'' which will never be used. Because $\Aut(\2^n)$ acts transitively on all of $\2^n \setminus \{0\}$ we can choose our bijections on $S$ to be group automorphisms. Then $\gamma_i \in \Z \times \Aut(S)$ and therefore we may set $\Gamma := \Z \times \Aut(S)$. Because in this case the sets $X_i$ as constructed in \ref{TMtoTDS} have only $n+1$ \todo{genauer} fixed components they are of the desired form.
	 
	Therefore the resulting Turing dynamical system is computing. \todo{stetig dazu sagen}
\end{Example}
...
Figure \ref{Masterplan} shows our procedure to proof this Theorem. The main effort will be the proof of Theorem \ref{groserSatz}.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[shorten >=1pt,on grid,auto]
		...
		;
	\end{tikzpicture}
	\caption{Procedure to prove \ref{HS}}
	\label{Masterplan}
\end{figure}
...
\end{Example}
We will always denote the set of objects of a groupoid $\mathcal{G}$ by $\mathcal{G}_0$ and we can identify it with a subset of the set of morphisms of $\mathcal{G}$ by the embedding $\textbf{1} \colon \mathcal{G}_0 \to mor(\mathcal{G}), x \mapsto [\id\colon x \to x]$ which sends every object to the corresponding identity morphism. Therefore we will only only look at $mor(\mathcal{G})$ and also call it $\mathcal{G}$. 

We will now give some definitions of basic properties of groupoids which we will use later in this chapter.

\begin{Definition}
	For every groupoid $\mathcal{G}$ we define the maps $s\colon \mathcal{G} \to \mathcal{G}_0, [f\colon X \to Y] \mapsto X$ and $r\colon\mathcal{G} \to \mathcal{G}_0, [f\colon X \to Y] \mapsto Y$ which will be called \emph{source} and \emph{range} map.
\end{Definition}
\begin{Definition}
	A \emph{relation groupoid} is a groupoid with $|mor(X, Y)| \leq 1 \ \forall X,Y \in \G_0$.
\end{Definition}
...
\begin{Definition}
	Let $\Phi\colon U \to \G$ be a measurable edge. The \emph{inverse} of $\Phi$ will be called $\Phi^{-1}$ and is defined as $\Phi^{-1}\colon r(Im(\Phi)) \to \G$ such that $\Phi^{-1}$ is a measurable edge and
	\begin{align*}
		\Phi^{-1} \circ r \circ \Phi (x) = \Phi (x)^{-1} \ \forall x \in r(Im(\Phi))
	\end{align*}
	where $\Phi (x)^{-1}$ is the inverse of the morphism $\Phi (x)$.
\end{Definition}

We now come to the definition of the groupoid ring. We want it to be a ring of operators of	$L^2(\G)$, \todo{Quelle fuer l2} such that it is in a way generated by measurable edges. For a measurable edge $\Phi$ we define an operator $\tilde \Phi$ by
\begin{align*}
	\tilde \Phi\colon L^2(\G) \to L^2(\G), F \mapsto \left[ \tilde \Phi (F)\colon\G \to \C, \gamma \mapsto \begin{cases}
...
\begin{align*}
	\tilde f\colon L^2(\G) \to L^2(\G), F \mapsto [\tilde f(F)\colon\G \to \C, \gamma \mapsto F(\gamma) \cdot f(r(\gamma))].
\end{align*}
...
\begin{Example} \label{groupoid_ring_of_groups}
	...
	On our measurable edges we define the isomorphism by mapping $\tilde{\Phi}$ of a measurable edge $\Phi\colon\bullet \mapsto g$ to $g$. This map is of course bijective. To see that it is a homeomorphism let $\Phi_1\colon\bullet \mapsto g_1$ and $\Phi_2\colon\bullet \mapsto g_2$ be measurable edges. It follows that
	\begin{align*}
	...
	\end{align*}
	which shows that the map maps $	\tilde{\Phi}_1 \cdot_{\C \G} \tilde{\Phi}_2$ to $g_1 \cdot_G g_2$ and therefore is indeed a homeomorphism.
\end{Example}
...
\end{Remark}
We will state the following fact without proof. For a complete proof see \cite{GRAB}.\todo{genaue Stelle}
...
\begin{proof}
	...
	\begin{align*}
		\tr(\sum_{(\hat{x}, \gamma) \in \hat{X} \rtimes_{\hat{\rho}} \Gamma} c_{(\hat{x}, \gamma)} \cdot \widetilde{P(\hat{x})} \cdot_{\C\G(\rho)} \widetilde{\bar{\gamma}}) = \sum_{(\hat{x}, e) \in \hat{X} \rtimes_{\hat{\rho}} \Gamma} \int_{\mathcal{G(\rho)}_0} c_{(\hat{x}, e)} P(\hat{x})(y) d\mu(y)
	\end{align*}
