...
\begin{proof}
	...
	tape. The second number refers to shifting the tape, e.g. a $1$ on the second coordinate means shifting the tape one to the left.\todo{akz rej Pfeile krumm}
	\begin{figure}[]
		\centering
		\begin{tikzpicture}[shorten >=1pt,on grid,auto]
		\node[state] (0) {$s_I$};
...
\begin{proof}
	...
	Figure \ref{Beispiel_dritte_TM} shows an example of the workflow of this Turing machine. The rightmost column shows the current state, the other rows show a port of the content of the $3$ tapes. The current position of the head (i.e. the element at the position with index $0$) is painted gray.
	\begin{figure}[]
		\centering
		\begin{tikzpicture}[shorten >=1pt,on grid,auto]
		\node[state] (0) {$s_I$};
