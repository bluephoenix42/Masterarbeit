...
\begin{figure}[H]
	...
	\begin{tikzpicture}[shorten >=1pt,on grid,auto]
		...
		\node[draw, rectangle, minimum width = \widthrectb, text width = \widthrectb, align = center] (6) [right =\abstrect of 2] {there exists an element $S$ in the groupoid ring with $\dim_{\mathcal{N}(\G(T_X))}(\ker S) = x$};
		...
		\path[->]
			...
			(6) edge [text width = 5 cm]
				node {using that $S$ lies in the image of $P \otimes 1$ as proven in the end of section \ref{image_of_P}}
				(7)
...
\counterwithin{Notation}{subsection} 
...
\begin{figure}[]
	...
	\begin{tikzpicture}[shorten >=1pt,on grid,auto]
	\node[state] (0) {$1$};
	\node[state] (1) [right = 3 of 0] {$2$};
	\node[state] (2) [below = 3 of 1] {$3$};
	
	\path[->]
	(0) edge [bend left, blue]
		node {$\Phi(1)$} 
	(1)
	(1) edge []
		node {} 
	(0)
	(1) edge [bend left, blue]
		node {$\Phi(2)$} 
	(2)
	(2) edge []
		node {} 
	...
	;
	\end{tikzpicture}
	\caption{Definition of $\Phi$.}
	\label{Graphik_groupoid_ring_1}
	\end{minipage}
	\begin{minipage}[t]{0.45\textwidth}
	\begin{tikzpicture}[shorten >=1pt,on grid,auto]
	\node[state] (0) {};
	\node[state] (1) [right = 3 of 0] {};
	\node[state] (2) [below = 3 of 1] {};
	
	\path[->]
	(0) edge [bend left, green]
		node {$F(\to)=1$} 
	(1)
	(1) edge [orange]
		node {$F(\to)=2$} 
	...
	node {} 
	(0)
	(1) edge [loop above, right, orange]
	node {$\tilde{\Phi}(F)(\to)=2$} 
	(1)
	(2) edge [loop below]
	node {} 
	...
	;
	\end{tikzpicture}
	\caption{Value of $\tilde{\Phi}(F)(x)$.}
	\label{Graphik_groupoid_ring_3}
\end{minipage}
\end{figure}
...
\begin{proof}
	...
	\begin{align*}
		...
		\end{cases}
	\end{align*}
	for all $i \in \{1 \ldots k\}$. Figure \ref{Graphik_erste_TM} shows an example of this Turing machine for the number $\frac{1}{16} = 0.0001$. In the figure the first number always indicates the number which the Turing machines reads on the
	...
	\begin{figure}[]
		...
		node {$(\1, 0)$} 
		(5)
		(4) edge [, bend left, below right]
		node {$(\0, 0)$} 
		(6)
		;
		\end{tikzpicture}
		\caption{Example of the first Turing machine for the number $\frac{1}{16}= 0.0001$.}
...
\begin{proof}
	...
	Figure \ref{Beispiel_dritte_TM} shows an example of the workflow of this Turing machine. The rightmost column shows the current state, the other rows show a part of the content of the $3$ tapes. The current position of the head (i.e. the element at the position with index $0$) is painted gray.
	\begin{figure}[]
		\centering
		\begin{tikzpicture}[shorten >=1pt,on grid,auto]
		\node[state] (0) {$s_I$};
		...
		;
		\end{tikzpicture}
		\caption{Transition function of the third Turing machine (The rejecting state is omitted).}
	...
	\begin{figure}[]
		...
		\begin{tabular}{|c|c|c|c|c|c|c|c||c|}\hline
			...
			$\1$\cellcolor[gray]{0.7} & $\0$ & $\0$ & $\1$ & $\0$ & $\0$ & $\0$ & $\0$ & $s_I$ \\
...
\begin{thebibliography}{Lam00}
 ...
 \bibitem{ALG} Aluffi, Paolo \emph{Algebra: Chapter 0}, in: Graduate Studies in Mathematics, Volume 104, American Mathematical Society, 2009
 \bibitem{FOLL} Folland, Gerald B.: \emph{A course in abstract harmonic analysis}, CRC Press, 1995
 \bibitem{BOR} Borceux, Francis: \emph{Handbook of categorical algebra 1}, Cambridge University Press, 1994
 \bibitem{ATI} Atiyah, Michael F.: \emph{Elliptic operators, discrete groups and von Neumann algebras}, in: Colloque ''Analyse et Topologie'' en l'Honneur de Henri Cartan pp. 43-72, Ast\'erisque 32-33, Soci\'et\'e Math\'ematique de France, 1976
\end{thebibliography}
